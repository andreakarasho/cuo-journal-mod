...
build:
	...
	# ruleset.replaces: the client takes its own system-log window down while this
	# mod is installed and enabled, so the two never stack (host side:
	# SystemLogGumpPlugin.ReplaceFeature).
